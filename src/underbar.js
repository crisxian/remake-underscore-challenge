...
(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(pass, item) {
      ...
      else return iterator(item)? true : false; // need bool
    }, true);
  };
  ...
  _.invoke = function(collection, functionOrKey, args) {
    return _.map(collection, function(value){
      ...
      else return functionOrKey.apply(value, args);
    });
  };
  ...
  _.zip = function() {
    ...
    var max = _.sortBy(args, "length")[args.length-1].length;
  ...
  _.flatten = function(nestedArray, result) {
    ...
    if(!Array.isArray(nestedArray)) {
      ...
      for(var each in nestedArray) {
        if(!Array.isArray(nestedArray[each])){
          result.push(nestedArray[each]);
        }
        else result = _.flatten(nestedArray[each], result);
      }
      return result;
    }
  };
  ...
  _.difference = function(array) {
    var other = Array.prototype.slice.call(arguments, 1, arguments.length);
    var results = [];
    var check;

    // compare array elements to other
    for(var i = 0; i < array.length; i++){
      check = false;
      for(var j = 0; j < other.length; j++){
        if(_.indexOf(other[j], array[i]) >= 0){
          check = true;
          break;
        }
      }
